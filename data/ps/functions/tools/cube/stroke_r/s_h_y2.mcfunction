...
execute if score #tools.cube.s.temp PS.mem matches 1 if score #tools.cube.p.y PS.mem >= #tools.cube.whd.h PS.mem positioned ~ ~-1 ~ unless entity @e[tag=PS.tools.cube.stroke,tag=PS.players.belonging,dx=0,dy=0,dz=0] run function ps:tools/cube/stroke_new
