...
#check markers
execute unless entity 0-0110-5053-0-1 run summon marker ~ ~ ~ {UUID:[I;0,17846355,0,1],Tags:["PS.","PS.focus.marker"],Marker:1b,Invisible:1b}
...
execute at @e[tag=PS.menu.focused] run summon snowball ~ ~.2 ~ {Tags:["PS.","PS.menu.cursor"],NoGravity:1b,Glowing:1b}

#clear tool items
kill @e[type=item,nbt={Item:{tag:{PS_tool:1b}}}]

#clear color raw
execute as @e[tag=PS.color.raw] if score @s PS.color.rawid > $color.maxraw PS.sys run kill @s
