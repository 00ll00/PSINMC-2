...
将笔刷的运动轨迹用实体标记记录，配合外部命令实现多种操作。绘制过程中会产生以下几种标记：

|标记|相关Tags|说明|
|-|-|-|
|落笔点|`PS.cstm.dn`, `PS.cstm`|在一次绘制中只会在落笔处生成一个|
|轨迹点|`PS.cstm.hd`, `PS.cstm`|绘制中每tick生成一个，其间距与笔刷运动速度有关|
|补间点|`PS.cstm.i`, `PS.cstm.hd`, `PS.cstm`|绘制中连接两个轨迹点的补间点，总是相邻|
|抬笔点|`PS.cstm.up`, `PS.cstm`|在一次绘制中只会在抬笔处生成一个|

这些标记仅会存在1tick，因此需要每tick执行外部命令以捕捉。

**使用方法**：在开始绘制之前需部署一些循环命令方块已定义绘制操作。部署完成后即可自由绘制。以下为一个示例：

> ```mcfunction
> execute as @e[tag=PS.cstm.dn] at @s run setblock ~ ~1 ~ minecraft:green_concrete
> ```
>
> ```mcfunction
> execute as @e[tag=PS.cstm.hd,tag=!PS.cstm.i] at @s run setblock ~ ~1 ~ minecraft:blue_concrete
> ```
>
> ```mcfunction
> execute as @e[tag=PS.cstm.up] at @s run setblock ~ ~1 ~ minecraft:red_concrete
> ```
>
> ```mcfunction
> execute as @e[tag=PS.cstm.i] at @s run setblock ~ ~ ~ minecraft:white_concrete
> ```
>
> 将以上四个命令分别放入四个循环命令方块中并开启，然后使用自定义可以绘制出如下效果：
>
> ![自定义画笔示例](./img/cstm_sample.png)

**注意事项**：

- 多人同时使用时应在命令中添加`if score @s PS.players.owner = <player> PS.players.id`以区分不同玩家，如：

```mcfunction
execute as @e[tag=PS.cstm] if score @s PS.players.owner = 00ll00 PS.players.id at @s run setblock ~ ~ ~ minecraft:red_concrete
```
